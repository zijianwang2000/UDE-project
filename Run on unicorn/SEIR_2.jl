...
save("Test data 2.jld", "X_test", X_test)
...
function train_ude(reg_ω, α_init, γ_init, adam_epochs, bfgs_epochs)
    ...
    function loss(x, p)   # x: optimized variable, p: parametrization of the objective
        ...
        elseif p.reg == "Integral"
            ...
            if size(X̂_fine) == size(X)
                mean(abs2, X_noisy .- X̂_fine[3:4, begin:5:end]) + p.ω * norm(Lux.apply(model, X̂_fine[:, begin:end-1], x.nn, st)[1], 1)
            else
                Inf
            end
        end
    ...
    save("Results 2/result_$(reg)_$(ω)_$(α_init)_$(γ_init)_$(adam_epochs)_$(bfgs_epochs)_$(array_nr).jld2", "result", result)
    return result
end 
...
function visualize(result) 
    ...
    dynamics_test = [-β/N * (X_test[1,:] .* X_test[3,:])'; β/N * (X_test[1,:] .* X_test[3,:])'; zeros(length(t))'; zeros(length(t))']
    ...
    save("Results 2/NRMSE_$(reg)_$(ω)_$(α_init)_$(γ_init)_$(adam_epochs)_$(bfgs_epochs)_$(array_nr).jld2", "nrmses", nrmses)
    ...
    savefig("Plots 2/Plot_$(reg)_$(ω)_$(α_init)_$(γ_init)_$(adam_epochs)_$(bfgs_epochs)_$(array_nr).pdf")
end
...
α_init_grid = round.(p_ode[1] .* (1.0/5.0, 1.0/2.0, 1.0/1.15, 1.0, 1.15, 2.0, 5.0), digits=2)
...
println("Experiment 2: $(reg_ω[1])_$(reg_ω[2])_$(α_init)_$(γ_init)_$(adam_epochs)_$(bfgs_epochs)_$(array_nr)")
